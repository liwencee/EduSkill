...
const VALUES = [
  { Icon: Target, title: 'Purposeful Learning', desc: 'Every course is designed with one goal: real employment and income for Nigerians.', color: 'text-[#378ADD]', bg: 'bg-blue-50' },
  { Icon: Heart, title: 'Radical Inclusion', desc: 'Works on 2G, in 5 languages, on any ₦15K Android phone. No Nigerian left behind.', color: 'text-[#F37321]', bg: 'bg-orange-50' },
...
export default function AboutPage() {
  return (
    <div className="min-h-screen bg-white font-sans">
      ...
      {/* Hero */}
      <section className="bg-gradient-to-br from-[#378ADD] via-[#3B82F6] to-[#60A5FA] text-white py-24 px-4 relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto text-center">
          ...
          <h1 className="font-heading text-5xl md:text-6xl font-bold mb-6 leading-tight">
            We Believe in a <span className="text-[#F37321]">Skilled Nigeria</span>
          </h1>
      ...
      <section className="py-20 bg-[#EBF4FF]">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <span className="inline-block bg-white border-[2px] border-blue-200 text-[#378ADD] font-bold text-sm px-5 py-2 rounded-full mb-5">Our Mission</span>
              <h2 className="font-heading text-4xl font-bold text-[#1E4F8A] mb-6">Upskill Teachers. Empower Youth. Connect Both to Opportunity.</h2>
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                { n: '48K+', l: 'Active Learners', c: 'text-[#378ADD]' },
                { n: '3.2K', l: 'Teachers Trained', c: 'text-purple-600' },
                { n: '12K+', l: 'Jobs Filled', c: 'text-green-600' },
                { n: '36', l: 'States Reached', c: 'text-[#F37321]' },
              ].map(s => (
                <div key={s.l} className="bg-white border-[3px] border-blue-100 rounded-3xl p-6 text-center shadow-[0_4px_0_rgba(55,138,221,0.1)]">
      ...
      <section className="py-20 bg-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            <h2 className="font-heading text-4xl font-bold text-[#1E4F8A] mb-4">What We Stand For</h2>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {VALUES.map(({ Icon, title, desc, color, bg }) => (
              <div key={title} className="bg-[#F8F7F2] border-[3px] border-blue-100 rounded-3xl p-6 hover:-translate-y-1 transition-all duration-200">
                <div className={`w-12 h-12 ${bg} rounded-2xl flex items-center justify-center mb-4`}>
                  <Icon className={`w-6 h-6 ${color}`} />
                </div>
                <h3 className="font-heading font-bold text-[#1E4F8A] text-lg mb-2">{title}</h3>
      ...
      <section className="py-20 bg-[#EBF4FF]">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            <h2 className="font-heading text-4xl font-bold text-[#1E4F8A] mb-4">Meet the Team</h2>
            <p className="text-gray-500 text-lg">Nigerians building for Nigerians.</p>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {TEAM.map(m => (
              <div key={m.name} className="bg-white border-[3px] border-blue-100 rounded-3xl p-6 text-center shadow-[0_4px_0_rgba(55,138,221,0.1)] hover:-translate-y-1 transition-all duration-200">
                <img src={m.avatar} alt={m.name} className="w-20 h-20 rounded-2xl object-cover mx-auto mb-4 border-[3px] border-blue-100" />
                <p className="font-heading font-bold text-[#1E4F8A] text-base">{m.name}</p>
                <p className="text-[#378ADD] text-sm font-medium mt-1">{m.role}</p>
              </div>
            ))}
          </div>
        </div>
