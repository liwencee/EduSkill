    ...
    <div className="min-h-screen bg-[#EBF4FF]">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-2xl font-bold text-[#1E4F8A]">My Certificates</h1>
        ...
        <div className="bg-white rounded-2xl border border-blue-100 shadow-sm p-12 text-center mb-8">
          <div className="w-20 h-20 bg-blue-50 rounded-3xl flex items-center justify-center mx-auto mb-5">
            <Award className="w-10 h-10 text-[#378ADD] opacity-40" />
          </div>
          <h2 className="text-xl font-bold text-[#1E4F8A] mb-2">No Certificates Yet</h2>
        ...
        <div>
          <h2 className="font-bold text-[#1E4F8A] text-lg mb-4">Courses with CPD Certificates</h2>
          <div className="grid sm:grid-cols-2 gap-4">
            {CPD_COURSES.map(c => (
              <Link
                key={c.slug}
                href={`/edupro/courses/${c.slug}`}
                className="bg-white rounded-2xl border border-blue-100 p-5 hover:shadow-md transition-all group flex items-start gap-4"
              >
                <div className="w-12 h-12 bg-[#EBF4FF] rounded-xl flex items-center justify-center text-2xl shrink-0">
                  {c.emoji}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-[#1E4F8A] text-sm group-hover:text-[#378ADD] transition-colors">{c.title}</p>
                  <p className="text-xs text-gray-500 mt-0.5">{c.weeks} weeks · {c.total_lessons} lessons · Pass mark: {c.pass_mark}%</p>
                </div>
                <ArrowRight className="w-4 h-4 text-gray-400 group-hover:text-[#378ADD] transition-colors shrink-0 mt-1" />
              </Link>
            ))}
          </div>
        </div>
